...
export class ClasificacionService {
  ...
  /* Create */
  async newClasificacion(datos: ClasificacionInput): Promise<ClasificacionResponse> {
    const clasificacion = this.clasificacionRepo.create({ nombre: datos.nombre });
    await this.clasificacionRepo.save(clasificacion);
    return this.toResponse(clasificacion);
  }

  /* Read */
  async getAllClasificaciones(page = 1, quantity = 10): Promise<ClasificacionResponse[]> {
  ...
  async getClasificacionById(id: number): Promise<ClasificacionResponse> {
    const clas = await this.clasificacionRepo.findOne({ where: { idClasificacion: id } });
    if (!clas) throw new Error('404 Clasificacion not found.');
    return this.toResponse(clas);
  }

  /* Update (full) */
  async updateClasificacion(id: number, datos: ClasificacionInput): Promise<ClasificacionResponse> {
    const clas = await this.clasificacionRepo.findOne({ where: { idClasificacion: id } });
    if (!clas) throw new Error('404 Clasificacion not found.');
